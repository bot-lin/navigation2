...
#include "nav2_waypoint_follower/plugins/drive_on_wp.hpp"
  ...
  const std::string & plugin_name, const std::string & params)
{
  ...
  speed_ = j["speed"];
  target_ = j["target"];
  this->driveon_client_ptr_ = rclcpp_action::create_client<nav2_msgs::action::DriveOnHeading>(
      node,
      "driveonheading");
...
bool DriveOnWp::processAtWaypoint(
  const geometry_msgs::msg::PoseStamped & /*curr_pose*/, const int & curr_waypoint_index)
{
  ...
  RCLCPP_INFO(
    ...
    curr_waypoint_index);
  if (!this->driveon_client_ptr_->wait_for_action_server()) {
  ...
  send_goal_options.result_callback =
      std::bind(&DriveOnWp::result_callback, this, std::placeholders::_1);
  this->driveon_client_ptr_->async_send_goal(goal_msg, send_goal_options);
  while(!is_done_){
    usleep(1000);
  }
  return true;
...
void DriveOnWp::result_callback(const rclcpp_action::ClientGoalHandle<nav2_msgs::action::DriveOnHeading>::WrappedResult & result)
{
...
PLUGINLIB_EXPORT_CLASS(
  nav2_waypoint_follower::DriveOnWp,
